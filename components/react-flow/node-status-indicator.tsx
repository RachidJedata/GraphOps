    ...
    <>
      ...
      `}
        ...
        <div className={cn("absolute inset-0 overflow-hidden rounded-md", className)}>
          <div className="spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,rgba(42,67,233,0.6)_0deg,rgba(42,138,246,0)_360deg)]" />
        </div>
      </div>
      {children}
    </>
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "error":
      return <StatusBorder className={cn("border-red-400/80", className)}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};
